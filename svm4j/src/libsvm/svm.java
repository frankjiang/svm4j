...
abstract class Kernel extends QMatrix
{
	...
	static double k_function(svm_node[] x, svm_node[] y, svm_parameter param)
	{
		switch (param.kernel_type)
		{
			...
			case svm_parameter.POLY:
				return Kernel.powi(
						param.gamma * Kernel.dot(x, y) + param.coef0,
						param.degree);
	...
	double kernel_function(int i, int j)
	{
		...
		{
			case svm_parameter.LINEAR:
				return Kernel.dot(x[i], x[j]);
			case svm_parameter.POLY:
				return Kernel.powi(gamma * Kernel.dot(x[i], x[j]) + coef0,
						degree);
			case svm_parameter.RBF:
				return Math.exp(-gamma
						* (x_square[i] + x_square[j] - 2 * Kernel.dot(x[i],
								x[j])));
			case svm_parameter.SIGMOID:
				return Math.tanh(gamma * Kernel.dot(x[i], x[j]) + coef0);
			case svm_parameter.PRECOMPUTED:
...
class Solver
{
	...
	double calculate_rho()
	{
		double r;
		int nr_free = 0;
		double ub = Solver.INF, lb = -Solver.INF, sum_free = 0;
	...
	void do_shrinking()
	{
		...
		double Gmax1 = -Solver.INF;
		// max { y_i * grad(f)_i | i in I_low(\alpha) }
		double Gmax2 = -Solver.INF;
	...
	{
		return alpha_status[i] == Solver.FREE;
	}

	boolean is_lower_bound(int i)
	{
		return alpha_status[i] == Solver.LOWER_BOUND;
	}

	boolean is_upper_bound(int i)
	{
		return alpha_status[i] == Solver.UPPER_BOUND;
	}
	...
	int select_working_set(int[] working_set)
	{
		...
		double Gmax = -Solver.INF;
		double Gmax2 = -Solver.INF;
		int Gmax_idx = -1;
		int Gmin_idx = -1;
		double obj_diff_min = Solver.INF;
	...
	{
		if (alpha[i] >= get_C(i))
			alpha_status[i] = Solver.UPPER_BOUND;
		else if (alpha[i] <= 0)
			alpha_status[i] = Solver.LOWER_BOUND;
		else
			alpha_status[i] = Solver.FREE;
	}
}
...
final class Solver_NU extends Solver
{
	...
	double calculate_rho()
	{
		...
		double ub1 = Solver.INF, ub2 = Solver.INF;
		double lb1 = -Solver.INF, lb2 = -Solver.INF;
	...
	void do_shrinking()
	{
		...
		double Gmax2 = -Solver.INF;
		// max { -y_i * grad(f)_i | y_i = -1, i in I_up(\alpha) }
		double Gmax3 = -Solver.INF;
		// max { y_i * grad(f)_i | y_i = -1, i in I_low(\alpha) }
		double Gmax4 = -Solver.INF;
		// find maximal violating pair first
		int i;
		for (i = 0; i < active_size; i++)
		{
	...
	int select_working_set(int[] working_set)
	{
		...
		// (if quadratic coefficeint <= 0, replace it with tau)
		// -y_j*grad(f)_j < -y_i*grad(f)_i, j in I_low(\alpha)
		double Gmaxp = -Solver.INF;
		double Gmaxp2 = -Solver.INF;
		int Gmaxp_idx = -1;
		double Gmaxn = -Solver.INF;
		double Gmaxn2 = -Solver.INF;
		int Gmaxn_idx = -1;
		int Gmin_idx = -1;
		double obj_diff_min = Solver.INF;
		for (int t = 0; t < active_size; t++)
...
public class svm
{
	...
	{
		if (svm.svm_print_string != null)
			svm.svm_print_string.print(s);
	}
			...
			svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for (i = 0; i < prob.l; i++)
		{
			int j = i + svm.rand.nextInt(prob.l - i);
			do
			{
				int _ = perm[i];
				perm[i] = perm[j];
			...
			svm_parameter param, int nr_fold, double[] target)
	{
				...
				&& nr_fold < l)
		{
			...
			svm.svm_group_classes(prob, tmp_nr_class, tmp_label, tmp_start,
					tmp_count, perm);
			...
			for (c = 0; c < nr_class; c++)
				for (i = 0; i < count[c]; i++)
				{
					int j = i + svm.rand.nextInt(count[c] - i);
					do
					{
		...
		for (i = 0; i < nr_fold; i++)
		{
			...
			{
				double[] prob_estimates = new double[svm
						.svm_get_nr_class(submodel)];
				for (j = begin; j < end; j++)
					target[perm[j]] = svm.svm_predict_probability(submodel,
			...
			throws IOException
	{
		...
		while (true)
		{
			...
			if (cmd.startsWith("svm_type"))
			{
				...
				if (i == svm.svm_type_table.length)
				{
			...
			else if (cmd.startsWith("kernel_type"))
			{
				...
				if (i == svm.kernel_type_table.length)
				{
	...
	public static svm_model svm_load_model(String model_file_name)
			throws IOException
	{
		return svm.svm_load_model(new BufferedReader(new FileReader(
				model_file_name)));
	}
			...
			double[] prob_estimates)
	{
				...
				&& model.probA != null && model.probB != null)
		{
			...
			for (i = 0; i < nr_class; i++)
				for (int j = i + 1; j < nr_class; j++)
				{
					pairwise_prob[i][j] = Math.min(Math.max(svm
			...
			throws IOException
	{
		...
		fp.writeBytes("svm_type " + svm.svm_type_table[param.svm_type] + "\n");
		fp.writeBytes("kernel_type " + svm.kernel_type_table[param.kernel_type]
				+ "\n");
	...
	{
		svm.svm_print_string = print_func;
	}
	...
	public static svm_model svm_train(svm_problem prob, svm_parameter param)
	{
		...
		else
		{
			...
			int[] count = tmp_count[0];
			if (nr_class == 1)
				svm.info("training data in only one class. See README for details.\n");
			...
			for (i = 0; i < nr_class; i++)
				for (int j = i + 1; j < nr_class; j++)
				{
					...
					if (param.probability == 1)
					{
						...
						svm.svm_binary_svc_probability(sub_prob, param,
								weighted_C[i], weighted_C[j], probAB);
						probA[p] = probAB[0];
						probB[p] = probAB[1];
					}
					f[p] = svm.svm_train_one(sub_prob, param, weighted_C[i],
							weighted_C[j]);
					for (k = 0; k < ci; k++)
